...
#:
#: The colon is the second half of that contract and was added when the workshop
#: met this renderer. ``app.workshop.sections`` reads a heading only when the
#: line carries a marker — ``#``, ``**bold**``, a trailing colon, or all caps —
#: and it requires one deliberately, so that a short line in a cover letter is
#: not mistaken for a section. Without it every rule the owner writes about a
#: named section of a CV silently measured nothing: ``в навыках не меньше 21
#: пункта`` could not fail, because the checker never found «Навыки» at all.
#: A colon after a section heading is ordinary in a Russian CV, the audit's
#: patterns match on ``search`` and are untroubled by it, and the alternative —
#: teaching the workshop to treat any short line as a heading — would weaken a
#: check on the letters as well.
