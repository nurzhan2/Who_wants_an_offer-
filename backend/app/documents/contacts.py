...
**Where the values come from, and where they will come from.** There is no
contact storage in this schema: ``candidate_profile`` holds a name, a city and a
languages list and no phone, email or links at all. They exist, though — inside
``raw_text``, which is the resume as the extractor read it — so this module
reads them back out at generation time. That is a stopgap with a known
replacement: the phase that adds an editable "Мои данные" screen puts these
fields in columns of their own, with a "corrected by hand" flag, and when it
lands :func:`contacts_of` reads the columns and this parsing becomes the
fallback for a profile that predates them. The seam is one function, on purpose.
...
def from_resume_text(
    ...
    quietly overrule that correction, which is the one thing a stopgap must not
    do — the phase that makes contacts editable has to be able to win.
