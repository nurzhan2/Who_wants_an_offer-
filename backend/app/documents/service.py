...
class DocumentOutcome:
    ...
    hard_rules: tuple[str, ...] = field(default_factory=rules.describe)
...
async def load_cv_context(
    ...
    return build_context(
        facts,
        profile,
        contacts=contacts.from_resume_text(
            await document_store.load_resume_text(session, profile.profile_id),
            name=profile.name,
    ...
    *,
    ...
    if context is None:
        return DocumentOutcome(
            vacancy_id=vacancy_id, kind=DocumentKind.CV, reason="vacancy_not_found"
        )
    if not context.experience:
        ...
        return DocumentOutcome(vacancy_id=vacancy_id, kind=DocumentKind.CV, reason="no_experience")

    try:
        generated = await generator.generate(context, router=router)
...
async def write_cover_letter(
    ...
    *,
    ...
    if context is None:  # pragma: no cover - the letter above already loaded it
        return DocumentOutcome(
            vacancy_id=vacancy_id, kind=DocumentKind.COVER_LETTER, reason="vacancy_not_found"
        )
