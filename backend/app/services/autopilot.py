...
letters, build the queue — from one press. It is long: a polite hh crawl is
about twenty minutes and embeddings on a CPU are another twenty, so it reports
per step rather than as one bar, stops at the step that failed and says which,
and a second press continues from there instead of re-crawling.
...
async def run_chain(steps: list[ChainStep]) -> list[str]:
    ...
    per step while a twenty-minute crawl is running.
...
async def _step_crawl(step: ChainStep) -> list[str]:
    ...
    step.note = (
        ...
        else "идёт обход источников — это самая долгая часть, около двадцати минут"
    )
    while True:
...
async def _stale_candidates(session: AsyncSession) -> list[UUID]:
    ...
    *only* remaining problem is that nobody has looked — see the ordering in
    ``app.services.agent_queue._set_aside`` — and the ready ones are re-read
    ...
    return list(dict.fromkeys([*stale, *ready]))
