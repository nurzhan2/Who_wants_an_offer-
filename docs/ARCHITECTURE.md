...
`BLOCKED_HOSTS` в транспорте отбивает LinkedIn, Indeed, Glassdoor, `hh.ru` и
`hh.kz` независимо от того, что объявил коннектор.
...
Считаются один раз, в конце, по уже дедуплицированным строкам, и никогда для
...
- `embedding.py` — батчевый расчёт векторов в конце прогона.
