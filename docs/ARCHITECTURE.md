...
GET    /api/v1/operations             последний запуск каждой операции и кто из них занят
POST   /api/v1/operations             кнопка «Обзора»: crawl|embed|match|letters|chain|outcomes|send
DELETE /api/v1/operations/{id}        отозвать запрос, который агент ещё не взял

GET    /api/v1/tracker/confirmations           что агент отправит по подтверждённому
GET    /api/v1/tracker/confirmations/{id}      карточка одной вакансии и её дайджест
POST   /api/v1/tracker/confirmations/{id}      подтвердить именно эту карточку
DELETE /api/v1/tracker/confirmations/{id}      отозвать подтверждение
GET    /api/v1/tracker/batch                   экран «Отправить все»: очередь, отсеянное, потолок
POST   /api/v1/tracker/batch                   N подтверждений, каждое под свой дайджест
```

**Автопилот: одна кнопка и одно подтверждение на пачку.** `POST /operations {kind: chain}`
запускает цепочку из пяти шагов — сбор, эмбеддинги, подбор, письма,
очередь — и отчитывается пошагово в `OperationRead.steps`, потому что шаги разной
длины: замеренный обход hh занимал здесь 79, 97 и 178 минут, а очередь собирается
за секунды. Шаг упал — цепочка встала и
назвала его; повторный запуск в течение двенадцати часов продолжает с него. То же
самое из терминала — `python -m wwao chain`, через эти же два эндпоинта.

Цепочка заканчивается готовой очередью и ничего не отправляет. `POST /tracker/batch`
записывает N подтверждений через ту же `services/confirmations.confirm`, что и
одиночная карточка, каждое под свой `card_digest`: пачка — это N мандатов на N
текстов, а не один мандат на список. Флага, снимающего это подтверждение, нет
нигде: ни в API, ни в CLI, ни в конфиге, и расписание `AUTOPILOT_DAILY_AT` запускает
только цепочку.

Отбор для пачки делает `services/agent_queue.triage`, и `build_queue` — его готовая
половина, чтобы экран и агент не могли видеть разные списки. Проверки идут в
порядке: ссылка совпадает с номером → не архив и не закрыта → разрыв по опыту
в пределе → язык не выше заявленного → письмо есть, проходит правила
мастерской и ATS-аудит → страницу перечитывали свежее `AGENT_PAGE_FRESHNESS_HOURS`.
Возраст страницы проверяется последним не для красоты: `stale_page` значит
«больше с ней ничего не так», и именно эти строки цепочка перечитывает через
`services/freshness` и `BaseSource.recheck`. Всё, что не прошло, едет в `set_aside` с
причиной и ссылкой — ничего не выбрасывается.
